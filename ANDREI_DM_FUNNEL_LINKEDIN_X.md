# Andrei Lucian DM Funnel · 10 messages

**LinkedIn** → ghostwriting · quiet founders · **€1,500 → €2,000/mo**  
**X** → 0→1K System · people who engage on your posts · **€47** (soon **€147**)

**Rule:** Each platform = **10 DMs max**. If they go quiet, you only send the follow-ups marked below. If they reply, skip ahead to the next conversation step (same day).

**Links**  
Ghostwriting https://www.andreilucian.com/ghostwriting  
0→1K X System https://www.andreilucian.com/0-to-1K-X-System/LANDING  
Free guide (Kit) https://andreilucian.kit.com/xsimplified  
Calendly https://calendly.com/theandreilucian/discovery-call  
Newsletter https://theandreilucian.substack.com

---

# LINKEDIN · 10 DMs

| # | When | Name |
|---|---|---|
| 1 | Day 0 | Connection request |
| 2 | +2 days after they accept | Opener |
| 3 | Same day they reply to #2 | Diagnose |
| 4 | After they answer #3 | Mirror |
| 5 | After #4 | Soft how you help |
| 6 | When they lean in | Website |
| 7 | When ready | Calendly |
| 8 | Only if they ask / on call | Price |
| 9 | If silent after #2: +3 days | Follow-up 1 |
| 10 | If still silent: +7 days after #9 | Follow-up 2 (STOP) |

**If they never reply:** you only send **1 → 2 → 9 → 10** then stop.  
**If conversation flows:** you send **1 → 2 → 3 → 4 → 5 → 6 → 7** (and **8** only if needed).

---

### DM 1 · Connection request (Day 0)
```
Hey [Name]! Quick note… I came across [Company] and liked what you're building.

I spend a lot of time around founders on LinkedIn and always enjoy connecting with people running real businesses. Would love to connect.
```

### DM 2 · Opener (+2 days after accept)
```
Hey [Name]! Thanks for connecting.

Saw you're running [Company]. Curious… is LinkedIn something you're actively using for the business this year, or have you mostly left it alone?
```

### DM 3 · Diagnose (same day they reply)
```
Nice.

How are you handling LinkedIn content right now? Writing it yourself when you can, or has it mostly stayed on the back burner?
```

### DM 4 · Mirror
```
Yeah, I hear that a lot.

It's rarely that founders don't care. They're just deep in the business, so LinkedIn stays empty for months.
```

### DM 5 · Soft how you help (no price)
```
Yeah, that's actually the gap I help with.

I work with founders who have a strong business but almost nothing on LinkedIn yet. We start from scratch: positioning, voice, then I turn their ideas into posts so they stay visible.

If useful, I can show you how it works. No pressure.
```

### DM 6 · Website
```
Here's a simple page that explains it:

https://www.andreilucian.com/ghostwriting

If anything's unclear, just ask me here.
```

### DM 7 · Calendly
```
If it'd help, we can jump on a quick 15 min call and I'll map what starting from scratch would look like for [Company].

https://calendly.com/theandreilucian/discovery-call

Only if useful. No deck. Just a clear picture.
```

### DM 8 · Price (only if asked / on call)
```
Standard is €2,000 a month.

For the first month I do €1,500. We build everything from scratch (positioning, voice, the weekly system). Then it goes to €2,000.

Easiest if we hop on a short call so I can see if it's even a fit:

https://calendly.com/theandreilucian/discovery-call
```

**On call close:**
```
Here's the simple version.

Month one is €1,500. We lock your positioning and voice, then I start shipping posts from your ideas / voice notes.

From month two it's €2,000/month.

You stay out of the blank page. I run the content side.

If that feels right, we can start this week.
```

### DM 9 · Follow-up 1 (no reply to opener · +3 days)
```
Hey [Name]! No worries if timing's off.

Quick yes/no… is personal brand on LinkedIn even on your radar this year?
```

### DM 10 · Follow-up 2 / last ( +7 days after DM 9 · then STOP)
```
All good. I'll leave it here. If you ever want 3 post angles for [Company], just say the word.
```

**Quiet mid-chat bump** (if they replied then went silent · +2 days, counts as using DM 9 energy):
```
Hey [Name]! No rush. Curious if any of that was useful, or should I leave it for now?
```

---

# X · 10 DMs

People who **comment / engage on your posts** → free guide → **0→1K X System (€47)** → **1:1 coaching** if they want more help.

| # | When | Name |
|---|---|---|
| 1 | Same day they engage | Free guide opener |
| 2 | Same day they reply | Diagnose |
| 3 | After #2 | Mirror |
| 4 | After #3 | Soft system bridge |
| 5 | They lean in | Landing page |
| 6 | Curious / asked | Offer + price |
| 7 | Want more help | 1:1 coaching upsell |
| 8 | Not ready | Soft exit |
| 9 | No reply to #1: +2 days | Follow-up 1 |
| 10 | Still silent: +5 days after #9 | Follow-up 2 (STOP) |

**If they never reply in DM:** **1 → 9 → 10** then stop.  
**If conversation flows:** **1 → 2 → 3 → 4 → 5 → 6** (then **7** if they want more help, or **8** if not ready).

---

### DM 1 · Free guide opener (same day they engage)
```
Hey friend,

Thank you for engaging with my post.

Here's your free guide:

andreilucian.kit.com/xsimplified

I hope you find it valuable.
```

### DM 2 · Diagnose (same day they reply)
```
Glad it helped.

Quick one — where are you at on 𝕏 right now? Just starting, stuck under a few hundred, or posting but the graph isn't moving?
```

### DM 3 · Mirror
```
Yeah, that's super common.

Usually it's not motivation. It's missing a simple daily system — what to post, who to engage, and how to stick to it when you don't feel like it.
```

### DM 4 · Soft system bridge
```
That's actually why I built a simple system for beginners to grow from 0 → 1K on 𝕏.

Daily plan, copy-paste templates, engagement scripts — so you stop guessing every morning.

Want me to send the page?
```

### DM 5 · Landing page
```
Here you go:

www.andreilucian.com/0-to-1K-X-System/LANDING

It's only €47 now — soon €147.

If anything's unclear, just ask me here.
```

### DM 6 · Offer + price (if they ask / lean in)
```
Hey! I made a simple system for beginners to grow from 0 → 1K on 𝕏.

If you know someone struggling to grow, send this to them:

www.andreilucian.com/0-to-1K-X-System/LANDING

It's only €47 now — soon €147.
```

### DM 7 · 1:1 coaching upsell (if they want more help)
```
If you want more help, I'm opening 1:1 coaching — to grow your personal brand and get clients as well.

The €47 system is the DIY path. Coaching is if you want me in your corner fixing your account with you.

Happy to hop on a quick call and map what I'd fix first:

https://calendly.com/theandreilucian/discovery-call
```

### DM 8 · Soft exit (not ready)
```
All good. Keep the free guide — that's enough to start:

andreilucian.kit.com/xsimplified

If you want the DIY system later:

www.andreilucian.com/0-to-1K-X-System/LANDING

And if you ever want 1:1 help growing your personal brand + getting clients, just say the word.
```

### DM 9 · Follow-up 1 (+2 days, no reply to free guide)
```
Hey! Just checking you got the guide.

andreilucian.kit.com/xsimplified

Any of it useful so far, or are you more stuck on what to post vs staying consistent?
```

### DM 10 · Follow-up 2 / last (+5 days after DM 9 · STOP)
```
All good — I'll leave it here.

If you ever want the full 0 → 1K daily system, it's here (still €47 for now):

www.andreilucian.com/0-to-1K-X-System/LANDING
```

---

# Daily volume

| Platform | Every day |
|---|---|
| LinkedIn | 10 connection requests (DM 1) · work open conversations |
| X | DM everyone who comments / engages · free guide first (DM 1) · work open conversations toward €47 |

---

# Sheet

`Name | Platform | DM # last sent | Date | Next DM # | Next date | Status`

Statuses: `1 sent → 2 due → Talking → Bought · Dead`

**That's the whole funnel: 10 DMs. No more.**
